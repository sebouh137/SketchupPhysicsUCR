...
{   
    ...
    rotate([0,0,90]) difference() {
        ...
        difference() {
            ...
            rotate([90,0,0]) translate([holePosition, 0, 0]) cylinder(h=2*thickness, r=hole_radius,center=true, $fn=32);
...
if(cblock_all_layers){
    ...
    for(layer_number=[0:bpc_nlayers-1])
        translate([cblock_position+bpc_layer_thickness*layer_number,0,0]) {
                cblock(hole_radius=bpc_hole_radius( layer_number),fn=12, holePosition=bpc_hole_position(layer_number), width=cblock_width-100);
                mirror([0,1,0]) cblock(hole_radius=bpc_hole_radius( layer_number), holePosition=-bpc_hole_position(layer_number), fn=12, width=cblock_width+100);
            }
} else {
